...
Int_t Fill_SingleCorrectionSpectra(
  ...
  TFile* f_EffCorrection = TFile::Open("EffMatrices_wCBSmear_fix.root", "READ");
  ...
  for (Int_t i = 0; i < entries; i++) {
    ...
    for (UInt_t j = 0; j < leptonIndex.size(); j++) {
      ...
      for (UInt_t k = j + 1; k < leptonIndex.size(); k++) {
        ...
        Int_t MombinCand1 = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetZaxis()->FindBin(Momentum1);
        Int_t MombinCand2 = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetZaxis()->FindBin(Momentum2);

        Int_t ThetabinCand1 = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetYaxis()->FindBin(Theta1);
        Int_t ThetabinCand2 = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetYaxis()->FindBin(Theta2);

        Int_t PhibinCand1 = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetXaxis()->FindBin(Phi1);
        Int_t PhibinCand2 = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetXaxis()->FindBin(Phi2);

        Float_t effCand1;

        if (Charge1 == 1) {
          effCand1 = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetBinContent(PhibinCand1, ThetabinCand1, MombinCand1);
        } else {
          effCand1 = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetBinContent(PhibinCand1, ThetabinCand1, MombinCand1);
        }

        Float_t effCand2;

        if (Charge2 == 1) {
          effCand2 = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetBinContent(PhibinCand2, ThetabinCand2, MombinCand2);

        } else {
          effCand2 = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetBinContent(PhibinCand2, ThetabinCand2, MombinCand2);
        }
    ...
    for (Int_t j = 0; j < sizePair; j++) {
      ...
      Int_t MombinCandA = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetZaxis()->FindBin(MomentumA);
      Int_t MombinCandB = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetZaxis()->FindBin(MomentumB);

      Int_t ThetabinCandA = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetYaxis()->FindBin(ThetaA);
      Int_t ThetabinCandB = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetYaxis()->FindBin(ThetaB);

      Int_t PhibinCandA = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetXaxis()->FindBin(PhiA);
      Int_t PhibinCandB = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetXaxis()->FindBin(PhiB);

      Float_t effCandA;

      if (LChargeA == 1) {
        effCandA = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetBinContent(PhibinCandA, ThetabinCandA, MombinCandA);
      } else {
        effCandA = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetBinContent(PhibinCandA, ThetabinCandA, MombinCandA);
      }

      Float_t effCandB;

      if (LChargeB == 1) {
        effCandB = h_Sm_matrix_kine_MomThetaPhi_ep[multClassNo-1]->GetBinContent(PhibinCandB, ThetabinCandB, MombinCandB);

      } else {
        effCandB = h_Sm_matrix_kine_MomThetaPhi_em[multClassNo-1]->GetBinContent(PhibinCandB, ThetabinCandB, MombinCandB);
      }
