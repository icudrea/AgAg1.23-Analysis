...
void draw_ScaledSpectra(TString sourceName = "pion") {
    ...
    // TString infileNameAcc = "~/lustre/hades/user/icudrea/mar19/loop/phd/pluto_files_ag123ag/pi0/pi0_IdealMom_SmearMom_LookUp_allCC.root";//for compare with niklas
    // TFile* file = new TFile(infileNameAcc, "READ");
    ...
    for (Int_t mult = 1; mult <= MAX_MULT; ++mult) {
        ...
        h_mass_acc[mult-1]    = (TH1D*) file->Get(Form("h_m_acc_%i", mult));
        h_mass_EffAcc[mult-1] = (TH1D*) file->Get(Form("h_m_recoCut_%i", mult));
        ...
        Float_t MultPerApart[5]         = {0.0515, 8e-4, 6.91e-6, 6.91e-6, 4.15e-8}; 
      ...
      // cout<<"multPions: "<<multPions<<endl;
        ...
        // TFile *outputFile = new TFile("~/march19AgAg/submit/EffCorrection/cocktail/pi0/pi0_SmearMomOnly_scaled_0515pi0_9opAngle100Mom.root", "RECREATE");
        //TFile *outputFile = new TFile("~/march19AgAg/submit/EffCorrection/cocktail/cocktail_correctedForpi0/pi0_SmearMomOnly_scaled_0515pi0_9opAngle100Mom.root", "RECREATE");
        outputFile->cd();
