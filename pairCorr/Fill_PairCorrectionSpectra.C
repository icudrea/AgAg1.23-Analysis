...
Int_t Fill_PairCorrectionSpectra(
  ...
  // TFile* f_PairCorrection    = TFile::Open("CorrectionFactor_centClass1_inmedRhoCG_9oAngle100Mom_100000000evt.root", "READ");
 

  //    if (!f_PairCorrection || f_PairCorrection->IsZombie()) {
  //     cout << "Error: HistogramEffFactor not found, DAMN" <<f_PairCorrection<<endl;
  //   }      

  // TH1D* EffFactor          = (TH1D*)f_PairCorrection->Get("h_EffCorr");
  // TH1D* AccFactor          = (TH1D*)f_PairCorrection->Get("h_AccCorr");

   

  //#################################################
  
  TFile* f_CentralityFiles[MAX_MULT];

  TH1D* EffFactor[MAX_MULT];
  TH1D* AccFactor[MAX_MULT];

const char* fileNames[MAX_MULT] = {
    "CorrectionFactor_centClass1_inmedRhoCG_9oAngle100Mom_100000000evt.root",
    "CorrectionFactor_centClass2_inmedRhoCG_9oAngle100Mom_100000000evt.root",
    "CorrectionFactor_centClass3_inmedRhoCG_9oAngle100Mom_100000000evt.root",
    "CorrectionFactor_centClass4_inmedRhoCG_9oAngle100Mom_100000000evt.root"};


for (Int_t mult = 1; mult <= MAX_MULT; ++mult) {
    f_CentralityFiles[mult-1] = TFile::Open(fileNames[mult-1], "READ");
    if (!f_CentralityFiles[mult-1]) {
        cout << "Error opening file: " << fileNames[mult-1] << endl;
        continue;
    }
    EffFactor[mult-1] = (TH1D*)f_CentralityFiles[mult-1]->Get("h_EffCorr");
    AccFactor[mult-1] = (TH1D*)f_CentralityFiles[mult-1]->Get("h_AccCorr");

    if (!EffFactor[mult-1] || !AccFactor[mult-1]) {
        cout << "Error retrieving histograms from file: " << fileNames[mult-1] << endl;
    }
}
 ...
 TH1D* fh_mass_ep_em_effAccCorr[MAX_MULT];
 TH1D* fh_mass_em_em_effAccCorr[MAX_MULT];
 TH1D* fh_mass_ep_ep_effAccCorr[MAX_MULT];
 ...
 TH1D* fh_mass_ep_em_mixedEvents_effAccCorr[MAX_MULT];
 TH1D* fh_mass_em_em_mixedEvents_effAccCorr[MAX_MULT];
 TH1D* fh_mass_ep_ep_mixedEvents_effAccCorr[MAX_MULT];
 ...
 //------------------------
  fh_mass_ep_em_effAccCorr[mult-1] = new TH1D(Form("fh_mass_ep_em_effAccCorr_%d",mult), "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  fh_mass_em_em_effAccCorr[mult-1] = new TH1D(Form("fh_mass_em_em_effAccCorr_%d",mult), "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  fh_mass_ep_ep_effAccCorr[mult-1] = new TH1D(Form("fh_mass_ep_ep_effAccCorr_%d",mult), "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
 ...
 //------------------------
  fh_mass_ep_em_mixedEvents_effAccCorr[mult-1] = new TH1D(Form("fh_mass_ep_em_mixedEvents_effAccCorr_%d",mult), "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  fh_mass_em_em_mixedEvents_effAccCorr[mult-1] = new TH1D(Form("fh_mass_em_em_mixedEvents_effAccCorr_%d",mult), "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  fh_mass_ep_ep_mixedEvents_effAccCorr[mult-1] = new TH1D(Form("fh_mass_ep_ep_mixedEvents_effAccCorr_%d",mult), "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
 ...
 //------------------------
  TH1D* h_mass_ep_em_effAccCorr = new TH1D("h_mass_ep_em_effAccCorr", "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  h_mass_ep_em_effAccCorr->GetXaxis()->SetTitle("M_{ee} [GeV/c^{2}]");
  h_mass_ep_em_effAccCorr->GetYaxis()->SetTitle("dN/dM_{ee} [1/(GeV/c^{2})]");

  TH1D* h_mass_em_em_effAccCorr = new TH1D("h_mass_em_em_effAccCorr", "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  h_mass_em_em_effAccCorr->GetXaxis()->SetTitle("M_{ee} [GeV/c^{2}]");
  h_mass_em_em_effAccCorr->GetYaxis()->SetTitle("dN/dM_{ee} [1/(GeV/c^{2})]");

  TH1D* h_mass_ep_ep_effAccCorr = new TH1D("h_mass_ep_ep_effAccCorr", "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  h_mass_ep_ep_effAccCorr->GetXaxis()->SetTitle("M_{ee} [GeV/c^{2}]");
  h_mass_ep_ep_effAccCorr->GetYaxis()->SetTitle("dN/dM_{ee} [1/(GeV/c^{2})]");
 ...
 //------------------------
   TH1D* h_mass_ep_em_mixedEvents_effAccCorr = new TH1D("h_mass_ep_em_mixedEvents_effAccCorr", "Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  h_mass_ep_em_mixedEvents_effAccCorr->GetXaxis()->SetTitle("M_{ee} [GeV/c^{2}]");
  h_mass_ep_em_mixedEvents_effAccCorr->GetYaxis()->SetTitle("dN/dM_{ee} [1/(GeV/c^{2})]");

  TH1D* h_mass_em_em_mixedEvents_effAccCorr = new TH1D("h_mass_em_em_mixedEvents_effAccCorr","Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  h_mass_em_em_mixedEvents_effAccCorr->GetXaxis()->SetTitle("M_{ee} [GeV/c^{2}]");
  h_mass_em_em_mixedEvents_effAccCorr->GetYaxis()->SetTitle("dN/dM_{ee} [1/(GeV/c^{2})]");

  TH1D* h_mass_ep_ep_mixedEvents_effAccCorr = new TH1D("h_mass_ep_ep_mixedEvents_effAccCorr","Invariant mass e^{+}e^{-} [GeV/c^{2}]", 360, 0., 1.8);
  h_mass_ep_ep_mixedEvents_effAccCorr->GetXaxis()->SetTitle("M_{ee} [GeV/c^{2}]");
  h_mass_ep_ep_mixedEvents_effAccCorr->GetYaxis()->SetTitle("dN/dM_{ee} [1/(GeV/c^{2})]");
  // parameters
   ...
   // if (i % 1000 == 0) cout << "event: " << i << endl;
    ...
    for (UInt_t j = 0; j < leptonIndex.size(); j++) {
      ...
      for (UInt_t k = j + 1; k < leptonIndex.size(); k++) {
        ...
        Int_t mass_bin_eff      = EffFactor[multClassNo-1]->GetXaxis()->FindBin( );
        Int_t mass_bin_low_eff  = EffFactor[multClassNo-1]->GetXaxis()->FindBin(0.015);//3
        Int_t mass_bin_high_eff = EffFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);//121
      
        if (mass_bin_eff  <= mass_bin_low_eff ) continue; //lower limit

        Int_t firstBin_eff = EffFactor[multClassNo-1]->GetXaxis()->FindBin(0.555);//112 
        Int_t lastBin_eff  = EffFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);///121

        Double_t integral_Eff = EffFactor[multClassNo-1]->Integral(firstBin_eff,lastBin_eff);
        Double_t averageBinContent_eff = integral_Eff/(lastBin_eff - firstBin_eff );
         
     

        if(mass_bin_eff > mass_bin_high_eff ){
        effW= 1/averageBinContent_eff; 
         ...
         if (effW > 10) continue;
        // cout<<"average_bin: "<< 1/averageBinContent_eff<<endl;
        // cout<<"bin_at0.6: "<<  1/EffFactor[multClassNo-1]->GetBinContent(121)<<endl;

        
        // if(mass_bin_eff > mass_bin_high_eff ){
        //   mass_bin_eff =integral_Eff/10;
        // }

        // effW =1/EffFactor->GetBinContent(mass_bin_eff);

      // cout<<"effW-candPair:  "<< effW  <<"  massBin: "<< mass_bin_eff<<endl;
      //-----------------------------------------------------------

       //-------Pair-acceptance weights----------------------------
        Float_t accW = 0;

        Int_t mass_bin_acc      = AccFactor[multClassNo-1]->GetXaxis()->FindBin(MassPair_scaled);
        Int_t mass_bin_low_acc  = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.015);//3
        Int_t mass_bin_high_acc = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);//121
      
        if (mass_bin_acc  <= mass_bin_low_acc ) continue; //lower limit

        Int_t firstBin_acc = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.555);//
        Int_t lastBin_acc  = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);///

    //  cout<<"1st : "<<firstBin_acc<<"2nd : "<<lastBin_acc<<endl;
        Double_t integral_Acc = AccFactor[multClassNo-1]->Integral(firstBin_acc,lastBin_acc);
        Double_t averageBinContent_Acc = integral_Acc/(lastBin_acc - firstBin_acc );
 

        if(mass_bin_acc > mass_bin_high_acc ){
        accW= 1/averageBinContent_Acc;                                 
        }
        else{
         accW = 1/AccFactor[multClassNo-1]->GetBinContent(mass_bin_acc);
        }
        ...
        // if(mass_bin_acc > mass_bin_high_acc ){
        //   mass_bin_acc =integral_Acc/10;
        // }
   
        // accW =1/AccFactor->GetBinContent(mass_bin_acc);

      // cout<<"accW-candPair:  "<< accW  <<"  massBinAcc: "<< mass_bin_acc<<endl;
      // cout<<" --------------------"<<endl;
      ...
      if ((oAngle > 9) && (Momentum1>100) && (Momentum2>100)) {
          if (Charge1 * Charge2 == -1) {
            ...
            fh_mass_ep_em_effAccCorr[multClassNo-1]->Fill(MassPair_scaled, effW*accW);

            h_mass_ep_em->Fill(MassPair_scaled);
            h_mass_ep_em_effCorr->Fill(MassPair_scaled, effW);
            h_mass_ep_em_effAccCorr->Fill(MassPair_scaled, effW*accW);
          }
          ...
          if (Charge1 == 1 && Charge2 == 1 ) {
            ...
            fh_mass_ep_ep_effAccCorr[multClassNo-1]->Fill(MassPair_scaled, effW*accW);

            h_mass_ep_ep->Fill(MassPair_scaled);
            h_mass_ep_ep_effCorr->Fill(MassPair_scaled, effW);
            h_mass_ep_ep_effAccCorr->Fill(MassPair_scaled, effW*accW);
          }
          ...
          if (Charge1 == -1 && Charge2 == -1) {
            ...
            fh_mass_em_em_effAccCorr[multClassNo-1]->Fill(MassPair_scaled, effW*accW);

            h_mass_em_em->Fill(MassPair_scaled);
            h_mass_em_em_effCorr->Fill(MassPair_scaled, effW);
            h_mass_em_em_effAccCorr->Fill(MassPair_scaled, effW*accW);
          }
        }
    ...
    for (Int_t j = 0; j < sizePair; j++) {
     ...
     //-------------Pair-efficiency correction EVENTMIX---------------------
      ...
      Int_t firstbin_eff = EffFactor[multClassNo-1]->GetXaxis()->FindBin(0.555);//111 
      Int_t lastbin_eff  = EffFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);///121
      ...
      Double_t averageBinContent_Eff = Integral_Eff/ (lastbin_eff - firstbin_eff );
        ...
        if(Mass_bin_eff > Mass_bin_high_eff ){
        effWeight = 1/averageBinContent_Eff;                                 
        }
        else{
        ...
        // effWeight      = 1/EffFactor->GetBinContent(Mass_bin_eff);


      // cout<<"effW-MEPair:  "<< effWeight  <<"  MassBin:  "<<  Mass_bin_eff<<endl;

      //-------------------------------------------------------------------------

      //-------------Pair-acceptance correction EVENTMIX---------------------
      Float_t accWeight = 0;

      Int_t Mass_bin_acc      = AccFactor[multClassNo-1]->GetXaxis()->FindBin(masspairScaled);
      Int_t Mass_bin_low_acc  = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.015);//bin no.3
      Int_t Mass_bin_high_acc = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);//  bin no.121
    
      if (Mass_bin_acc  <= Mass_bin_low_acc ) continue; //lower limit

  
      Int_t firstbin_acc = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.555);//bin no. 111 
      Int_t lastbin_acc  = AccFactor[multClassNo-1]->GetXaxis()->FindBin(0.6);/// bin no. 121
     
      Double_t Integral_Acc = AccFactor[multClassNo-1]->Integral(firstbin_acc,lastbin_acc);
      Double_t averageBinContent = Integral_Acc / (lastbin_acc - firstbin_acc );


        if(Mass_bin_acc > Mass_bin_high_acc ){
        accWeight = 1/averageBinContent;   
                                      
        }
        else{
         accWeight = 1/AccFactor[multClassNo-1]->GetBinContent(Mass_bin_acc);
        }
        
  
     // cout<<"accW-MEPair:  "<< accWeight  <<"  MassBin:  "<<  Mass_bin_acc<<endl;
      // cout<<" --------------------"<<endl;
       ...
       if ((oAngle > 9) && (MomentumA>100) && (MomentumB>100)) {
        ...
        if (LChargeA * LChargeB == -1) {
          ...
          fh_mass_ep_em_mixedEvents_effAccCorr[multClassNo-1]->Fill(masspairScaled, effWeight*accWeight);

          h_mass_ep_em_mixedEvents->Fill(masspairScaled);
          h_mass_ep_em_mixedEvents_effCorr->Fill(masspairScaled, effWeight);
          h_mass_ep_em_mixedEvents_effAccCorr->Fill(masspairScaled, effWeight*accWeight);
        }
        ...
        if (LChargeA == 1 && LChargeB == 1) {
          ...
          fh_mass_ep_ep_mixedEvents_effAccCorr[multClassNo-1]->Fill(masspairScaled, effWeight*accWeight);

          h_mass_ep_ep_mixedEvents->Fill(masspairScaled);
          h_mass_ep_ep_mixedEvents_effCorr->Fill(masspairScaled, effWeight);
          h_mass_ep_ep_mixedEvents_effAccCorr->Fill(masspairScaled, effWeight*accWeight);
        ...
        if (LChargeA == -1 && LChargeB == -1) {
          ...
          fh_mass_em_em_mixedEvents_effAccCorr[multClassNo-1]->Fill(masspairScaled, effWeight*accWeight);

          h_mass_em_em_mixedEvents->Fill(masspairScaled);
          h_mass_em_em_mixedEvents_effCorr->Fill(masspairScaled, effWeight);
          h_mass_em_em_mixedEvents_effAccCorr->Fill(masspairScaled, effWeight*accWeight);
 ...
 for (Int_t mult = 1; mult <= MAX_MULT; ++mult) {
  ...
  fh_mass_ep_em_effAccCorr[mult-1] ->Write();
  fh_mass_ep_ep_effAccCorr[mult-1] ->Write();
  fh_mass_em_em_effAccCorr[mult-1] ->Write();
  ...
  fh_mass_ep_em_mixedEvents_effAccCorr[mult-1]->Write();
  fh_mass_ep_ep_mixedEvents_effAccCorr[mult-1]->Write();
  fh_mass_em_em_mixedEvents_effAccCorr[mult-1]->Write();
 }
  ...
  h_mass_ep_em_effAccCorr ->Write();
  h_mass_ep_ep_effAccCorr ->Write();
  h_mass_em_em_effAccCorr->Write();
  ...
  h_mass_ep_em_mixedEvents_effAccCorr->Write();
  h_mass_ep_ep_mixedEvents_effAccCorr->Write();
  h_mass_em_em_mixedEvents_effAccCorr->Write();
