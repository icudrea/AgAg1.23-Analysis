...
void rho_invmass_plutoCocktail( int centClass = 4){
 TString infileName ="~/lustre/hades/dstsim/mar19/ag1230ag/pluto/vertex/w/w";
   ...
   TF1* params_em[] = {Param_0_em, Param_1_em, Param_2_em, Param_3_em,
	...
	for( long i=0; i<nentries; i++ ) {         // enter event loop
		...
		if( ep && em ) {
			...
			//float mom_ep = Smearing(params_ep, ep->P()*1000.);
			//float mom_em = Smearing(params_em, em->P()*1000.);
	...
	TFile* outfile = new TFile( Form("~/lustre/hades/user/icudrea/mar19/loop/phd/pluto_files_ag123ag/w/histSmearMom_centClass%d_w_1000000evt_pluto.root", centClass ), "RECREATE" );
	//TFile* outfile = new TFile( Form("~/lustre/hades/user/fseck/pluto_files_ag123ag/w/histSMEAR_centClass%d_w_1000000evt_pluto.root", centClass ), "RECREATE" );
