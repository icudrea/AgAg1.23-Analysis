...
void rho_invmass_plutoCocktail( int centClass = 4){
 TString infileName ="~/lustre/hades/dstsim/mar19/ag1230ag/pluto/vertex/pi0/pi0";
   ...
   TF1* params_em[] = {Param_0_em, Param_1_em, Param_2_em, Param_3_em,
	...
	TString infileNameAcc = "/u/icudrea/march19AgAg/submit/EffCorrection/AccMatrix_wCBSmear_fix.root";
	TFile* infileAcc = new TFile( infileNameAcc, "READ" );


	TString infileNameAccIdeal = "/u/icudrea/lustre/hades/user/icudrea/mar19/loop/phd/SingleCorrection/AccMatrix_wIdealSmear_fix.root";
	TFile* infileAccIdeal = new TFile( infileNameAccIdeal, "READ" );

	// x-axis: phi in degree, y-axis: theta in degree, z-axis: momentum in MeV/c^2

	//pick-up smearing values
	...
	//pick-up ideal values

	TH3D* h_acc_ep = ( TH3D* ) infileAccIdeal->Get( Form( "h_matrix_kine_MomThetaPhi_ep_%d", centClass ) );
	TH3D* h_acc_em = ( TH3D* ) infileAccIdeal->Get( Form( "h_matrix_kine_MomThetaPhi_em_%d", centClass ) );
	...
	TString infileNameEff = "/u/icudrea/march19AgAg/submit/EffCorrection/EffMatrix_WLowMomCB_Rebin_30_fix.root";
	TFile* infileEff = new TFile( infileNameEff, "READ" );

	TString infileNameEffIdeal = "/u/icudrea/lustre/hades/user/icudrea/mar19/loop/phd/SingleCorrection/EffMatrix_wIdealSmear_fix.root";
	TFile* infileEffIdeal = new TFile( infileNameEffIdeal, "READ" );


	// x-axis: phi in degree, y-axis: theta in degree, z-axis: momentum in MeV/c^2

	//pick-up smearead values
	...
	//pick-up ideal values
	TH3D* h_eff_ep = ( TH3D* ) infileEffIdeal->Get( Form( "h_matrix_kine_MomThetaPhi_ep_%d", centClass ) );
	TH3D* h_eff_em = ( TH3D* ) infileEffIdeal->Get( Form( "h_matrix_kine_MomThetaPhi_em_%d", centClass ) );
	...
	TH1F* h_m_smear   = new TH1F( "h_m_smear",   "", 360, 0., 1.8 );
	TH1F* h_m_ideal   = new TH1F( "h_m_ideal",   "", 360, 0., 1.8 );
	h_m_smear  ->Sumw2();
	h_m_ideal  ->Sumw2();
    

	// set up histograms in acc
	TH1F* h_m_inacc_smear   = new TH1F( "h_m_inacc_smear",   "", 360, 0., 1.8 );
	TH1F* h_m_inacc_ideal   = new TH1F( "h_m_inacc_ideal",   "", 360, 0., 1.8 );
	h_m_inacc_smear  ->Sumw2();
	h_m_inacc_ideal  ->Sumw2();


	// set up histograms in acc & eff -> reco
	TH1F* h_m_reco_smear   = new TH1F( "h_m_reco_smear",   "", 360, 0., 1.8 );
	TH1F* h_m_reco_ideal   = new TH1F( "h_m_reco_ideal",   "", 360, 0., 1.8 );
	h_m_reco_smear  ->Sumw2();
	h_m_reco_ideal  ->Sumw2();
	

	// set up histograms in acc & eff + opening angle cut -> recoOpang
	
	//TH1F* h_m_recoOpang   = new TH1F( "h_m_recoOpang",   "",nBins, binArray);
	TH1F* h_m_recoOpang_smear   = new TH1F( "h_m_recoOpang_smear",   "", 360, 0., 1.8 );
	TH1F* h_m_recoOpang_ideal   = new TH1F( "h_m_recoOpang_ideal",   "", 360, 0., 1.8 );

	TH1F* h_m_acc_opAng_smear   = new TH1F( "h_m_acc_opAng_smear",   "", 360, 0., 1.8 );
	TH1F* h_m_acc_opAng_ideal   = new TH1F( "h_m_acc_opAng_ideal",   "", 360, 0., 1.8 );
   
	h_m_recoOpang_smear  ->Sumw2();
	h_m_recoOpang_ideal  ->Sumw2();

	h_m_acc_opAng_smear  ->Sumw2();
	h_m_acc_opAng_ideal  ->Sumw2();

	// set up histograms in acc & eff + opening angle and momentum cut -> recoCut
	TH1F* h_m_recoCut_smear   = new TH1F( "h_m_recoCut_smear",   "", 360, 0., 1.8 );
	TH1F* h_m_recoCut_ideal   = new TH1F( "h_m_recoCut_ideal",   "", 360, 0., 1.8 );

	TH1F* h_m_acc_ideal       = new TH1F( "h_m_acc_ideal",   ""    , 360, 0., 1.8 );  
	TH1F* h_m_acc_smear       = new TH1F( "h_m_acc_smear",   ""    , 360, 0., 1.8 );    
	
	h_m_recoCut_smear  ->Sumw2();
	h_m_recoCut_ideal  ->Sumw2();
	...
	for( long i=0; i<nentries; i++ ) {         // enter event loop
		...
		if( ep && em ) {
			...
			//smearing momentum
			float momSmear_ep = Smearing(h_crystalball_ep, ep->P() * 1000. );
			float momSmear_em = Smearing(h_crystalball_em, em->P() * 1000. );
            ...
            float epE_smeared = sqrt( pow(momSmear_ep / 1000., 2. ) + pow(ep->M(), 2.) );
            float emE_smeared = sqrt( pow(momSmear_em / 1000., 2. ) + pow(em->M(), 2.) );

			ep_smear.SetE( epE_smeared );
     		em_smear.SetE( emE_smeared );

			ep_smear.SetRho( momSmear_ep / 1000. );
     		em_smear.SetRho( momSmear_em / 1000. );
			...
			h_m_inacc_smear  ->Fill( mass_smear, pair_weight_acc );
			h_m_reco_smear   ->Fill( mass_smear,  pair_weight_acc * pair_weight_eff );//can be reconstructed 
			

			if( opang_smear > 9. ) {
				h_m_recoOpang_smear  ->Fill(mass_smear,pair_weight_acc * pair_weight_eff );
				h_m_acc_opAng_smear  ->Fill(mass_smear,pair_weight_acc);
			}

			if( opang_smear > 9. && momSmear_ep > 100. && momSmear_em > 100. ) {
				h_m_recoCut_smear  ->Fill(mass_smear,pair_weight_acc * pair_weight_eff );
				h_m_acc_smear      ->Fill(mass_smear,pair_weight_acc);
			...
			h_m_ideal  	     ->Fill( mass );
			h_m_inacc_ideal  ->Fill( mass, pair_weight_acc );
			h_m_reco_ideal  ->Fill( mass,  pair_weight_acc * pair_weight_eff );//can be reconstructed 
			

			if( opang_smear > 9. ) {
				h_m_recoOpang_ideal  ->Fill(mass,pair_weight_acc * pair_weight_eff );
				h_m_acc_opAng_ideal  ->Fill(mass,pair_weight_acc);
			}

			if( opang_smear > 9. && momSmear_ep > 100. && momSmear_em > 100. ) {
				h_m_recoCut_ideal  ->Fill(mass,pair_weight_acc * pair_weight_eff );
				h_m_acc_ideal      ->Fill(mass,pair_weight_acc);
	...
	TFile* outfile = new TFile( Form("~/lustre/hades/user/icudrea/mar19/loop/phd/pluto_files_ag123ag/pi0/histIdealMom_SmearMom_LookUp_centClass%d_pi0_1000000evt_pluto.root", centClass ), "RECREATE" );
	
	h_m_smear            ->Write();
	h_m_inacc_smear      ->Write();
	h_m_reco_smear       ->Write();
	h_m_recoOpang_smear  ->Write();
	h_m_recoCut_smear    ->Write();
	h_m_acc_smear        ->Write();
	h_m_acc_opAng_smear  ->Write();


	h_m_ideal            ->Write();
	h_m_inacc_ideal      ->Write();
	h_m_reco_ideal       ->Write();
	h_m_recoOpang_ideal  ->Write();
	h_m_recoCut_ideal    ->Write();
	h_m_acc_ideal        ->Write();
	h_m_acc_opAng_ideal  ->Write();
}